// Implement the interface IEventStore
// over a struct pg which have a *gorm.DB connection pool

// NewPostgresEventStore constructor that takes the Postgres connection string
// sets up the GORM connection pool with a logger attached to it, that will log all the queries executed
// Returns the PostgreSQL implementation of IEventStore instead of the pg struct

package store

import (
	"context"
	"github.com/youlimko725/rest-api-go-postgres/errors"
...
func NewPostgresEventStore(conn string) IEventStore {
	...
	if err := db.AutoMigrate(&objects.Event{}); err != nil {
		panic("Unable to migrate to database: " + err.Error())
	}
